...
const InputField = ({ label, id, type, value, onChange, loading, icon: Icon }) => (
...
const PasswordField = ({ label, id, value, onChange, show, toggle, loading }) => (
    <div className="space-y-1">
        <label htmlFor={id} className="block text-sm font-medium text-gray-700">{label}</label>
        <div className="relative">
            <input
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const { user, accessToken, tokenExpiresAt } = data;

            if (accessToken && tokenExpiresAt && user) {
    ...
    <div className="flex min-h-screen bg-gray-100 items-center justify-center p-4 sm:p-0">
        <div className="bg-white rounded-xl shadow-2xl flex max-w-5xl w-full overflow-hidden">
            ...
            <div className="w-full lg:w-1/2 p-8 sm:p-12 md:p-16 flex flex-col justify-center">
                ...
                {/* Logo */}
                <div className="mb-10">
                    <img src={stnLogo} alt="STN Logo" className="h-32 mb-6 object-contain" />
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* Status Messages */}
                    {error && <p className="text-red-600 text-sm font-medium p-2 rounded bg-red-50 text-center mt-4">{error}</p>}
                    {success && <p className="text-green-600 text-sm font-medium p-2 rounded bg-green-50 text-center mt-4">{success}</p>}
            ...
            <div className="hidden lg:flex lg:w-1/2 relative">
                ...
                <div className="relative z-10 flex flex-col justify-center items-center text-white p-8 text-center">
                    <h3 className="text-5xl font-extrabold mb-4 leading-tight">Access Your Business Hub</h3>
                    <p className="text-xl max-w-md">
                        Sign in to manage inventory, track orders, and gain real-time insights into your operations. Your efficiency starts now.
                    </p>
                </div>
            </div>
        </div>
